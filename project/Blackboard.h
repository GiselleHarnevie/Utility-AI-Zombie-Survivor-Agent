...
public:
	...
	//for debugs
	void PrintBlackboard() const
	{
		printf("=== Blackboard Contents ===\n");
		for (const auto& pair : m_BlackboardData)
		{
			printf("Key: %s", pair.first.c_str(), typeid(*pair.second).name());

			// Try to print value for common types
			if (auto p = dynamic_cast<BlackboardField<int>*>(pair.second))
				printf("%d", p->GetData());
