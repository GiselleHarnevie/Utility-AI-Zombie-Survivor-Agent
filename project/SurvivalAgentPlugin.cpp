...
void SurvivalAgentPlugin::Initialize(IBaseInterface* pInterface, PluginInfo& info)
{
	...
	m_pSurvivorAgentBrain = std::make_unique<SurvivorAgentBrain>(m_pInterface);
	m_pSurvivorAgentBrain->Initialize();



	//Information for the leaderboards!
	info.BotName = "Mini";
	info.Student_Name = "Giselle_Harnevie"; //No special characters allowed. Highscores won't work with special characters.
	info.Student_Class = "2DAE10";
	info.LB_Password = "!";//Don't use a real password! This is only to prevent other students from overwriting your highscore!
}
...
SteeringPlugin_Output SurvivalAgentPlugin::UpdateSteering(float dt)
{
	...
	bool autoOrient{};
	bool isRunning{};
	m_pSurvivorAgentBrain->GetBlackboard()->GetData("AutoOrient", autoOrient);
	m_pSurvivorAgentBrain->GetBlackboard()->GetData("IsRunning", isRunning);
	steering.AutoOrient = autoOrient; //Setting AutoOrient to true overrides the AngularVelocity
	steering.RunMode = isRunning; //If RunMode is True > MaxLinearSpeed is increased for a limited time (until your stamina runs out)
	//std::cout << "AUTOOREINT" << steering.AutoOrient<< std::endl;

	SteeringOutput steeringOutput = m_pSurvivorAgentBrain->Update(dt, agentInfo);

	steering.LinearVelocity = steeringOutput.LinearVelocity;
	steering.AngularVelocity = steeringOutput.AngularVelocity;
	//std::cout << "curr Linear Speed" << m_pInterface->Agent_GetInfo().CurrentLinearSpeed << " Grab rabge " << m_pInterface->Agent_GetInfo().GrabRange << std::endl;
	//@End (Demo Purposes)
...
void SurvivalAgentPlugin::Render(float dt) const
{
	...
	m_pSurvivorAgentBrain->Render(dt);
